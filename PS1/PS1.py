...
#%% 1.5 Rank your households by income, and dicuss the behavior of the top and bottom of the
#           consumption and wealth distributions conditional on income.
df['quantile'] = pd.qcut(df['inctotal'], 5, labels=False)    
df0 = df[df["quantile"]==0]
df4 = df[df["quantile"]==4]
df0.describe() ## Descriptive stat of bottom 20% income earners
df4.describe() ## Descriptive stat of top 20% income earners
...
sns.jointplot(x="ctotal", y="totW", data=df0)
plt.title('Bottom 20%')
plt.savefig('now1.jpeg')

import seaborn as sns
sns.jointplot(x="ctotal", y="totW", data=df4)
plt.title('Top 20%')
plt.savefig('now2.jpeg')

sns.distplot(df0["ctotal"],hist=False , label='Consumptiom', kde=True)
sns.distplot(df0["inctotal"],hist=False , label='Income', kde=True)
sns.distplot(df0["totW"],hist=False , label='Wealth', kde=True)
plt.legend(loc='upper right')
plt.xlabel('CIW level ')
plt.ylabel('Probability')
plt.title('Based on Income of Bottom 20%')
plt.grid(True)
plt.savefig('now3.jpeg')
plt.show()

sns.distplot(df4["ctotal"],hist=False , label='Consumptiom', kde=True)
sns.distplot(df4["inctotal"],hist=False , label='Income', kde=True)
sns.distplot(df4["totW"],hist=False , label='Wealth', kde=True)
plt.legend(loc='upper right')
plt.xlabel('CIW level ')
plt.ylabel('Probability')
plt.title('Based on Income of Top 20%')
plt.grid(True)
plt.savefig('now4.jpeg')
plt.show()
